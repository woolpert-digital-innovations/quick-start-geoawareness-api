...
const getStores = async () => {
    const storesFile = fs.readFileSync('./example-responses/getStores.json', 'utf8');
    return JSON.parse(storesFile);
};

const getGeofencesByStore = async storeName => {
    try {
        const geofencesFile = fs.readFileSync(`./example-responses/getGeofencesByStoreName-${storeName.toLowerCase()}.json`, 'utf8');
...
const getOrdersByStore = async storeName => {
    try {
        const ordersFile = fs.readFileSync(`./example-responses/getOrdersByStoreName-${storeName.toLowerCase()}.json`, 'utf8');
